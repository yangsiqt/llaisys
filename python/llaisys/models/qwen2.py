...
class Qwen2:
    ...
    def __init__(self, model_path, device: DeviceType = DeviceType.CPU):
        ...
        maxseq = min(config.get("max_position_embeddings", 131072), 4096)  # Limit for memory
        ...
        for file in sorted(model_path.glob("*.safetensors")):
            ...
            for name, weight_torch in weight_dict.items():
                ...
                if name == "model.embed_tokens.weight":
                    ...
                    tensor = Tensor(tuple(weight_np.shape), dtype=self._dtype, device=self._device)
                    ...
                    tensor = Tensor(tuple(weight_np.shape), dtype=self._dtype, device=self._device)
                    tensor.load(weight_np.ctypes.data)
                    LIB_LLAISYS.llaisysQwen2ModelSetOutEmbed(self._model, tensor._tensor)
                    ...
                    tensor = Tensor(tuple(weight_np.shape), dtype=self._dtype, device=self._device)
                    tensor.load(weight_np.ctypes.data)
                    LIB_LLAISYS.llaisysQwen2ModelSetOutNormW(self._model, tensor._tensor)
                    self._weight_tensors.append(tensor)
                    ...
                    if weight_name == "input_layernorm.weight":
                        LIB_LLAISYS.llaisysQwen2ModelSetLayerWeight(self._model, b"attn_norm_w", layer_idx, tensor._tensor)
                        self._weight_tensors.append(tensor)
                    elif weight_name == "self_attn.q_proj.weight":
                        LIB_LLAISYS.llaisysQwen2ModelSetLayerWeight(self._model, b"attn_q_w", layer_idx, tensor._tensor)
                        self._weight_tensors.append(tensor)
                    elif weight_name == "self_attn.q_proj.bias":
                        LIB_LLAISYS.llaisysQwen2ModelSetLayerWeight(self._model, b"attn_q_b", layer_idx, tensor._tensor)
                        self._weight_tensors.append(tensor)
                    elif weight_name == "self_attn.k_proj.weight":
                        LIB_LLAISYS.llaisysQwen2ModelSetLayerWeight(self._model, b"attn_k_w", layer_idx, tensor._tensor)
                        self._weight_tensors.append(tensor)
                    elif weight_name == "self_attn.k_proj.bias":
                        LIB_LLAISYS.llaisysQwen2ModelSetLayerWeight(self._model, b"attn_k_b", layer_idx, tensor._tensor)
                        self._weight_tensors.append(tensor)
                    elif weight_name == "self_attn.v_proj.weight":
                        LIB_LLAISYS.llaisysQwen2ModelSetLayerWeight(self._model, b"attn_v_w", layer_idx, tensor._tensor)
                        self._weight_tensors.append(tensor)
                    elif weight_name == "self_attn.v_proj.bias":
                        LIB_LLAISYS.llaisysQwen2ModelSetLayerWeight(self._model, b"attn_v_b", layer_idx, tensor._tensor)
                        self._weight_tensors.append(tensor)
                    elif weight_name == "self_attn.o_proj.weight":
                        LIB_LLAISYS.llaisysQwen2ModelSetLayerWeight(self._model, b"attn_o_w", layer_idx, tensor._tensor)
                        self._weight_tensors.append(tensor)
                    elif weight_name == "post_attention_layernorm.weight":
                        LIB_LLAISYS.llaisysQwen2ModelSetLayerWeight(self._model, b"mlp_norm_w", layer_idx, tensor._tensor)
                        self._weight_tensors.append(tensor)
                    elif weight_name == "mlp.gate_proj.weight":
                        LIB_LLAISYS.llaisysQwen2ModelSetLayerWeight(self._model, b"mlp_gate_w", layer_idx, tensor._tensor)
                        self._weight_tensors.append(tensor)
                    elif weight_name == "mlp.up_proj.weight":
                        LIB_LLAISYS.llaisysQwen2ModelSetLayerWeight(self._model, b"mlp_up_w", layer_idx, tensor._tensor)
                        self._weight_tensors.append(tensor)
                    elif weight_name == "mlp.down_proj.weight":
                        LIB_LLAISYS.llaisysQwen2ModelSetLayerWeight(self._model, b"mlp_down_w", layer_idx, tensor._tensor)
                        self._weight_tensors.append(tensor)
