...
Runtime::Runtime(llaisysDeviceType_t device_type, int device_id)
    ...
    // A stream is owned by the current device context.  Creating it before
    // selecting the device happens to work with CUDA's default context, but
    // produces an invalid/cross-context stream with AscendCL.
